    ...
    <main className="analyze">
      <div className="analyze-inner">
        <div className="analyze-head">
          <h1>생기부 분석</h1>
          <p>생활기록부를 붙여넣으면 AI가 진단 리포트를 만들어 드립니다.</p>
        </div>

        <form className="panel input-card" onSubmit={onSubmit}>
          ...
          <div className="actions">
            <button type="button" className="btn btn-ghost" onClick={() => setRecordText(SAMPLE)} disabled={loading}>
              예시 넣기
            </button>
            <button
              type="submit"
              className="btn btn-primary"
              disabled={loading || recordText.trim().length < 30}
            >
              {loading ? "분석 중…" : "분석하기"}
            </button>
          </div>
